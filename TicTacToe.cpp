...
void SetKinkByMouse(int button, int state, int x, int y)//마우스함수 이용해서 구간 나눠서 x좌표가 0~100이면 세모, 101~200이면 네모, 201~300이면 원으로
{
	...
	{
		if (x>0 && x<101 && y < 150 && kind2 != 1) //세모 선택했을 경우      
			kind1 = 1;
		else if (x>100 && x<201 && y<150 && kind2 != 1)//네모 선택했을 경우 
			kind1 = 2;
		else if (x>200 && x<300 && y<150 && kind2 != 1)//원 선택했을 경우  
			kind1 = 3;
...
int main(int argc, char **argv)
{
	...
	glutInitWindowSize(300, 350); //윈도우 사이즈는 x=300, y=350    
